...
private:
    static std::string generateOrderId(int instrumentId, const std::string& traderId) {
        ...
        // Extract trade count from traderId (format: USER_X where X is the trade count)
        int tradeCount = 0;
        if (traderId.substr(0, 5) == "USER_") {
            tradeCount = std::stoi(traderId.substr(5));
        }
        
        // Format: instrumentId-random10DigitNumber-tradeCount
        return std::to_string(instrumentId) + "-" + 
